...
//! HF fallback / smart source / resumability are deferred to S2.
...
pub fn modelscope_url() -> String {
    format!("https://www.modelscope.cn/models/OpenBMB/MiniCPM5-1B-GGUF/resolve/master/{MODEL_FILE}")
}
...
pub enum DownloadError {
    ...
    #[error("download incomplete: got {got} bytes")] Incomplete { got: u64 },
}
...
mod tests {
    ...
    fn url_targets_modelscope_q4km() {
        ...
        assert!(u.contains("modelscope.cn/models/OpenBMB/MiniCPM5-1B-GGUF"));
        assert!(u.ends_with(MODEL_FILE));
    }
}
