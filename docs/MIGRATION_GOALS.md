...
| **R4** | 工具/MCP/模型（F5） | 🟡 基础+跨运行时桥已证（2026-05-30） | R3 ✅ | 150K | **F5 + 跨运行时桥模式已证（绿）**：`tool_output_to_result`（pi ToolOutput→renderer string）+ `UclawToolFactory`（继承 pi 8 内置 verbatim + 注入点）+ **`ExitPlanTool`（首个包装 uClaw 工具，`impl pi::sdk::Tool`，execute() 在 asupersync 经 R3 `ApprovalRegistry` 往返前端，端到端闭环：tool→`agent:exit_plan_request`→对话框→`respond_exit_plan_mode`→`EngineCmd::Respond`→registry→`ToolOutput`）** + set_model 机制。25 引擎测试。**IO 桥接缝已全接**：`tool_bridge.rs`（`ToolRequestSink`+`ToolResultRegistry`+`BridgedIoTool`，4 测试）+ `PiEngine::spawn` 接 `ToolRequestSink` + `EngineCmd::ToolResult` 解析 + 工厂按 `io_tool_specs()` 造 `BridgedIoTool` + main.rs `StubToolRequestSink`（缝已通，executor 待接）。29 引擎测试。**待续**：换 stub 为真实 tokio executor（mcp.rs/browser/skill 回路 + 声明 io_tool_specs）；api_key→`SessionOptions.api_key`(F7)；live 证据 |
...
- v1.21 (2026-05-30): **R4 IO 桥接缝全接（steps 1–3）**（`f71d3426`）。① `PiEngine::spawn` 增 `Option<Arc<dyn ToolRequestSink>>` 参；`run_engine_thread`/`actor_loop` 串进；actor_loop 建 `ToolResultRegistry`、解析 `EngineCmd::ToolResult{request_id,text,is_error}`→`tool_output_text`→唤醒 `BridgedIoTool::execute()`。② `ToolRequestSink` 增 `io_tool_specs()`（uClaw 声明自己的 IO 工具名+schema，引擎不耦合具体工具）；`UclawToolFactory` 持 `ToolResultRegistry`+`Option<sink>`，按 specs 逐个 push `BridgedIoTool`。③ uClaw `StubToolRequestSink`（engine_sink.rs，声明 0 IO 工具+log 请求）；main.rs `PiEngine::spawn(sink, Some(stub), config)`。**缝端到端连通**（pi 调 IO 工具→BridgedIoTool.execute() register→sink.request()→[真实 executor]→EngineCmd::ToolResult→registry→ToolOutput），仅差真实 tokio executor 替换 stub。29 引擎测试绿、`cargo check -p uclaw` 0 错。
- v1.20 (2026-05-30): **R4 IO 工具桥基础设施（tokio-resolver 变体）**（`3d69402a`）。`tool_bridge.rs`：`ToolRequestSink`（engine→tokio，抽象同 EventSink）+ `ToolResultRegistry`（register oneshot→await `ToolOutput`，超时 fail-closed 成 error output，永不挂）+ `BridgedIoTool`（`impl pi::sdk::Tool`，execute() 先 register 再 dispatch 再 await，避免快速回复丢失）。**4 测试绿**（respond/唯一 id/超时 fail-closed/往返），引擎共 **29 测试**。至此**两种桥变体核心基础设施均备**：交互工具（前端 resolver，ExitPlanTool 已端到端证）+ IO 工具（tokio resolver，引擎侧完整+测试）。**剩余集成**：`EngineCmd::ToolResult` 解析 + `PiEngine::spawn` 接 `ToolRequestSink` + main.rs 的 uClaw tokio executor 回路真实 mcp.rs/browser/skill 工具 + live 证据。
- v1.19 (2026-05-30): **R4 跨运行时工具桥 已起步并端到端证明**（`26f44fa9`+`37f4582b`）。首个包装 uClaw 工具 **`ExitPlanTool`**（`impl pi::sdk::Tool`，`#[async_trait]`）：`execute()` 在 pi **asupersync** runtime 跑，经 R3 `ApprovalRegistry` **往返前端**——register（先于 emit，保证快速 respond 命中）→ emit `agent:exit_plan_request` → await 用户决策（Allow=accept/Deny=reject）→ `ToolOutput`；`decision_to_output` 经 `tool_output_to_result` flatten 成 renderer string（接 Done-when #1）。`UclawToolFactory.create_tool_registry` push 它（与 pi verbatim 内置并存）。前端半闭环：`respond_exit_plan_mode`→`EngineCmd::Respond{request_id, allow:decision!=reject, reason:feedback}`→registry 解析（`RespondExitPlanInput` 已带 `request_id`，契约不变）。**端到端**：pi 调 exit_plan_mode→tool emit+await→对话框→respond→Respond→registry→ToolOutput。25 引擎测试（+metadata +decision-mapping）。**这是所有包装 uClaw 工具的桥模式**；IO 工具（MCP/browser/skill）同形，仅 resolver 由前端换 **tokio executor**（`ToolRequestSink` engine→tokio + `EngineCmd::ToolResult` + 真实工具回路 + live）——为下一增量。
